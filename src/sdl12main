...
static const SDL_Color base_palette[16] = {{0x00, 0x00, 0x00}, {0x1d, 0x2b, 0x53}, {0x7e, 0x25, 0x53}, {0x00, 0x87, 0x51}, {0xab, 0x52, 0x36}, {0x5f, 0x57, 0x4f}, {0xc2, 0xc3, 0xc7}, {0xff, 0xf1, 0xe8}, {0xff, 0x00, 0x4d}, {0xff, 0xa3, 0x00}, {0xff, 0xec, 0x27}, {0x00, 0xe4, 0x36}, {0x29, 0xad, 0xff}, {0x83, 0x76, 0x9c}, {0xff, 0x77, 0xa8}, {0xff, 0xcc, 0xaa}};
...
int main(int argc, char** argv) {
	...
	{
		const unsigned char loading_bmp[] = {};
...
static void mainLoop(void) {
	...
	if (!TAS) {
		...
		if (kbstate[SDLK_DOWN])
			buttons_state |= (1 << 3);
		if (kbstate[SDLK_z] || kbstate[SDLK_c] || kbstate[SDLK_n] || kbstate[SDLK_a])
			buttons_state |= (1 << 4);
		if (kbstate[SDLK_x] || kbstate[SDLK_v] || kbstate[SDLK_m] || kbstate[SDLK_b])
			buttons_state |= (1 << 5);
	} else if (TAS && !paused) {
		static int t = 0;
		t++;
...
#define CASE(t, ...)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
	...
	switch (call) {
		CASE(
			CELESTE_P8_MUSIC, // music(idx,fade,mask)
			int index = INT_ARG();
			int fade = INT_ARG(); int mask = INT_ARG();
		...
		CASE(
			CELESTE_P8_SPR, // spr(sprite,x,y,cols,rows,flipx,flipy)
			int sprite = INT_ARG();
			int x = INT_ARG(); int y = INT_ARG(); int cols = INT_ARG(); int rows = INT_ARG(); int flipx = BOOL_ARG(); int flipy = BOOL_ARG();
		...
		CASE(CELESTE_P8_SFX, // sfx(id)
			 ...
			 if (id < (sizeof snd) / (sizeof *snd) && snd[id]) Mix_PlayChannel(-1, snd[id], 0);)
		CASE(
			CELESTE_P8_PAL, // pal(a,b)
			int a = INT_ARG();
			int b = INT_ARG(); if (a >= 0 && a < 16 && b >= 0 && b < 16) {
				// swap palette colors
		...
		CASE(CELESTE_P8_PAL_RESET, // pal()
			 ResetPalette();)
		CASE(
			CELESTE_P8_CIRCFILL, // circfill(x,y,r,col)
			int cx = INT_ARG() - camera_x;
			int cy = INT_ARG() - camera_y; int r = INT_ARG(); int col = INT_ARG();
		...
		CASE(
			CELESTE_P8_PRINT, // print(str,x,y,col)
			const char* str = va_arg(args, const char*);
			int x = INT_ARG() - camera_x; int y = INT_ARG() - camera_y; int col = INT_ARG() % 16;
		...
		CASE(CELESTE_P8_RECTFILL, // rectfill(x0,y0,x1,y1,col)
			 ...
			 int y0 = INT_ARG() - camera_y; int x1 = INT_ARG() - camera_x; int y1 = INT_ARG() - camera_y; int col = INT_ARG();
		...
		CASE(
			CELESTE_P8_MAP, // map(mx,my,tx,ty,mw,mh,mask)
			int mx = INT_ARG(), my = INT_ARG();
			int tx = INT_ARG(), ty = INT_ARG(); int mw = INT_ARG(), mh = INT_ARG(); int mask = INT_ARG();
...
static int gettileflag(int tile, int flag) { return tile < sizeof(tile_flags) / sizeof(*tile_flags) && (tile_flags[tile] & (1 << flag)) != 0; }
